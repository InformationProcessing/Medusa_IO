...
def getarray():
      ...
      welcome_socket.bind(('172.28.164.169',server_port))
      ...
      for i in range(len(temparray)):
            array.append([temparray.split(",")[0],temparray.split(",")[1]])
...
def tick(player):
    ...
    getarray()#should be coordinates taken from tcp
    ...
    #Displays Other Player Blocks 
    #Other Player is a 2D Array which contains other blocks and each array inside contains individual blocks of other users
    for i in range(len(otherplayer)):
              for j in range(len(otherplayer[i])):
                    otherplayerblocks.append(canvas.create_rectangle(otherplayer[i][j][0],otherplayer[i][j][1],otherplayer[i][j][0]+10,otherplayer[i][j][1]+10))
