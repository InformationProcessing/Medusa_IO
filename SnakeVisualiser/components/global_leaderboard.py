...
class GlobalLeaderboard:
    def __init__(self, root, total_score, highest_score):
        ...
        self.buttons_frame = ttk.Frame(root, padding="30 0 10 0")
        self.buttons_frame.grid(column=0, row=1, sticky=(N, E))
        self.__create_change_order_button("Sort by total score", self.sort_by_total_score)
        self.leaderboard_frame = ttk.Frame(root, padding="0 0 0 40")
        self.leaderboard_frame.grid(column=0, row=2, sticky=(N, E))
    ...
    def __create_change_order_button(self, text, command):
        ...
        self.change_order_button.grid(column=0, row=0, sticky=(N, W, E))
        self.exit_game_button = Button(self.buttons_frame, text="Exit game & close window",
                                            command=self.root.quit)
        self.exit_game_button.grid(column=1, row=0, sticky=(N, W, E))

    def __set_title_label(self, text):
        if self.title_label is not None:
            self.title_label.destroy()
        self.title_label = ttk.Label(self.root, text=text)
        self.title_label.grid(column=0, row=0, sticky = (N,E))
        self.title_label.grid_configure(padx=20, pady=20)
